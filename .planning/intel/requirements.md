...
> Extracted from SPECs (functional, AI, frontend, API) and from the ADR's two new MVP features.
> Each entry includes a stable `REQ-{slug}` ID, source path(s), description, and acceptance criteria.
> The two ADR-introduced AI features (REQ-ai-inline-help, REQ-ai-comparative-analysis) carry
> precedence-locked status from `UPDATE-001-pulse-rebrand-ai-features.md` (see decisions.md DEC-004, DEC-005).
...
### REQ-ai-inline-help (NEW — locked by DEC-004)
...
### REQ-ai-comparative-analysis (NEW — locked by DEC-005)
...
## L. AI Assistant — Phase 2 (deferred from MVP)
...
### REQ-no-evidence-upload (locked by DEC-010)
...
### REQ-pulse-branding (locked by DEC-001)
...
### REQ-pulse-heartbeat-animation (locked by DEC-003)
